...
const fetchUserData = async (userId, type) => {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch ${type} for user ${userId}: ${response.statusText}`,
    );
    return;
  }
...
const generateAccordianEl = (id, headerInfoTemplate) => {
  ...
  item
    .querySelector(".accordian-header")
    .addEventListener("click", handleAccordianToggle);
  return item;
};
...
const setAccordianExpand = (accordianPanel, buttonPanel, detailsPanel, expand) => {
    accordianPanel?.setAttribute("data-expanded", expand.toString());
    buttonPanel?.setAttribute("aria-expanded", expand.toString());
    detailsPanel?.setAttribute("aria-hidden", (!expand).toString());
};
...
const handleAccordianToggle = async (event) => {
  ...
  if (isExpanded) {
    setAccordianExpand(selectedAccordian, trigger, detailsPanel, false);
    return;
  }

  setAccordianExpand(selectedAccordian, trigger, detailsPanel, true);
...
const renderTodos = (detailsPanel, todos = []) => {
  if (todos.length === 0) {
    detailsPanel.innerHTML = generateErrorMarkup(
...
const init = async () => {
  ...
  try {
    ...
    await renderUserOneByOne(data.users);
